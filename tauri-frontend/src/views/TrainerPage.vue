    ...
    <div class="workspace-wide-frame wide-workspace-grid grid h-full min-h-0 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_clamp(360px,24vw,430px)]">
      <aside class="wide-workspace-sidebar hidden min-h-0 flex-col gap-5 border-r border-slate-200 bg-[#f8fafc] px-5 py-6 dark:border-zinc-800 dark:bg-zinc-950">
        <header>
          ...
          <h2 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">偏好训练</h2>
          <p class="mt-1 text-xs leading-5 text-slate-500 dark:text-zinc-400">用样片训练符合个人审美的本地模型</p>
        </header>
        ...
        <button type="button" @click="selectDirectory"
          ...
          <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-600 dark:bg-blue-950/60 dark:text-blue-400"><FolderOpen class="h-4 w-4" /></span>
          <span class="min-w-0 flex-1">
            <span class="block truncate text-xs font-semibold text-slate-900 dark:text-zinc-100">{{ folderName }}</span>
            <span v-if="photosDir" class="mt-1 block break-all text-[10px] leading-4 text-slate-400 dark:text-zinc-500">{{ photosDir }}</span>
            <span v-else class="mt-1 block text-[10px] leading-4 text-slate-400 dark:text-zinc-500">包含 like 和 dislike 子文件夹</span>
            <span class="mt-2 block text-[10px] font-medium text-blue-600 dark:text-blue-400">{{ photosDir ? "更换目录" : "浏览选择" }}</span>
          </span>
        </button>
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-3.5 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <div class="space-y-2.5 text-[11px] text-slate-500 dark:text-zinc-400">
            <div class="flex items-center gap-2"><CheckCircle2 class="h-3.5 w-3.5 shrink-0 text-emerald-500" />本地训练，不上传样片</div>
            <div class="flex items-center gap-2"><ShieldCheck class="h-3.5 w-3.5 shrink-0 text-blue-500" />样片始终保留在本机</div>
          </div>
        </section>
      </aside>
      ...
      <section class="flex min-w-0 min-h-0 flex-col border-r border-slate-200 bg-[#f8fafc] dark:border-zinc-800 dark:bg-zinc-950">
        <div class="workspace-main-scroll flex-1 px-7 py-6">
          <div class="mx-auto flex min-h-full w-full max-w-6xl flex-col gap-5">
            ...
            <div class="wide-workspace-flow flex h-[260px] shrink-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-slate-900 shadow-[0_12px_36px_rgba(15,23,42,0.12)] dark:border-zinc-800">
          ...
          <div>
            ...
            <div class="rounded-lg border border-slate-200 bg-slate-50 px-3 pb-2 pt-4 dark:border-zinc-700 dark:bg-zinc-800/70">
              <input
                ...
                :style="{ '--range-progress': `${epochSliderPosition / 10}%` }"
