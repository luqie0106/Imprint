    ...
    def _build_ui(self) -> None:
        ...
        if self.is_standalone:
            ...
            tk.Label(hdr, text="📷  RAW 连拍优选", bg=_BG, fg=_TEXT,
                     font=(_FAM_TITLE, 20, "bold")).pack(anchor="w")
        ...
        self.model_status_frame = tk.Frame(mc, bg=_SURFACE, padx=14, pady=8)
        ...
        di = tk.Frame(dc, bg=_SURFACE, padx=14, pady=10)
        di.pack(fill="x")

        _label(di, "📁  RAW 照片目录", bold=True, size=11).pack(anchor="w", pady=(0, 4))
        row = tk.Frame(di, bg=_SURFACE)
        row.pack(fill="x")
        _entry(row, self.input_dir_var).pack(side="left", fill="x",
                                             expand=True, ipady=4, padx=(0, 8))
        _pill_button(row, "选择目录", self._pick_dir).pack(side="left")

        # ── 3. 参数配置卡片（精简双列排版）──
        pc = _card(body)
        pc.pack(fill="x", pady=(0, 10))
        pi = tk.Frame(pc, bg=_SURFACE, padx=14, pady=10)
        pi.pack(fill="x")

        _label(pi, "⚙️  处理参数配置", bold=True, size=11).pack(anchor="w", pady=(0, 8))
        ...
        # 第 0 行：淘汰目录名 + 保留张数
        tk.Label(g, text="淘汰子目录名称:", bg=_SURFACE, fg=_TEXT_DIM, font=(_FAM, 10)).grid(
            row=0, column=0, sticky="w", padx=(0, 6), pady=3)
        _entry(g, self.review_subdir_var, width=16).grid(row=0, column=1, sticky="w", padx=(0, 16), pady=3)

        tk.Label(g, text="每组保留张数:", bg=_SURFACE, fg=_TEXT_DIM, font=(_FAM, 10)).grid(
            row=0, column=2, sticky="w", padx=(0, 6), pady=3)
        _entry(g, self.keep_count_var, width=8).grid(row=0, column=3, sticky="w", pady=3)

        # 第 1 行：时间阈值 + 最大线程
        tk.Label(g, text="连拍时间阈值(秒):", bg=_SURFACE, fg=_TEXT_DIM, font=(_FAM, 10)).grid(
            row=1, column=0, sticky="w", padx=(0, 6), pady=3)
        _entry(g, self.gap_var, width=16).grid(row=1, column=1, sticky="w", padx=(0, 16), pady=3)

        max_cpus = os.cpu_count() or 4
        tk.Label(g, text="最大并发线程数:", bg=_SURFACE, fg=_TEXT_DIM, font=(_FAM, 10)).grid(
            row=1, column=2, sticky="w", padx=(0, 6), pady=3)
        self._workers_entry = _entry(g, self.workers_var, width=8)
        self._workers_entry.grid(row=1, column=3, sticky="w", pady=3)
        self.workers_var.trace_add("write", self._on_workers_changed)

        # 第 2 行：汉明距离 + 硬件加速
        tk.Label(g, text="dHash 汉明限制:", bg=_SURFACE, fg=_TEXT_DIM, font=(_FAM, 10)).grid(
            row=2, column=0, sticky="w", padx=(0, 6), pady=3)
        _entry(g, self.hamming_var, width=16).grid(row=2, column=1, sticky="w", padx=(0, 16), pady=3)

        tk.Checkbutton(g, text="启用 显卡/NPU 硬件加速",
                       variable=self.gpu_var, bg=_SURFACE, fg=_TEXT, font=(_FAM, 10),
                       activebackground=_SURFACE).grid(row=2, column=2, columnspan=2, sticky="w", pady=3)
        ...
        self.progress = ttk.Progressbar(br, mode="indeterminate", length=180)
    ...
    def refresh_model_status(self):
        ...
        if status.onnx_ready:
            self.model_status_lbl.configure(
                text="🟢 AI 美学引擎：ONNX 极速硬件加速已就绪 (已启用)",
                fg=_SUCCESS
            )
        elif status.mlp_ready:
            self.model_status_lbl.configure(
                text="🟡 AI 美学引擎：PyTorch 模式 (建议在“模型与环境”熔铸 ONNX)",
                fg=_WARNING
            )
        else:
            self.model_status_lbl.configure(
                text="⚪ AI 美学引擎：未检测到模型 (当前降级为纯 OpenCV 锐度 + 曝光过滤)",
                fg=_TEXT_DIM
            )
